...
const IMPULSE_MAX: float = 1200.0 # used in arrow sprite scaling -> len of impulse vector / this max to give % to scale arrow sprite 
...
func scale_arrow() -> void:
	var imp_len = get_impulse().length() # Vector2
...
func _on_sleeping_state_changed():
	if sleeping:
		...
		if cb.size() > 0:
			cb[0].die() # only one cup at a time
		call_deferred("die")
